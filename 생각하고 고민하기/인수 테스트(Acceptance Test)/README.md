...
(그래서 과정이름이 `인수 테스트 + TDD` = `ATDD` 으로 알고있습니다.)
...
우선, 간단하게 도메인은 확인해보겠습니다.  

### 지하철 도메인 설명

![지하철 도메인](https://github.com/hbkuk/blog/assets/109803585/ed82b11f-d5a0-47da-9cfa-65212fccbb1a)  
[참고: ATDD, 클린 코드 with Spring 강의 자료 中 ...](https://edu.nextstep.camp/c/R89PYi5H)

위 그림에서 표현된, 지하철 도메인은 중 `지하철 역(Station)`에 대해서만 이해하고 계시면 될 것 같습니다.
...
일반적으로 지하철 역에 대해서 `생성(Create)`, `조회(Read)`, `삭제(Delete)` 기능을 구현해본다고 가정해보겠습니다.

그렇다면, 각 기능에 대해서 `domain` -> `Repository` -> `Service` -> `Controller` 계층을 순차적으로 설계해볼 것 같아요.
...
인수 테스트에서는 기능을 구현하기 전, 이해당사자와 충분하게 **인수 조건을 정의**합니다.
...
웹 서버를 구동시킨 후, 어떻게 테스트 코드에서 `HTTP 기반 API 요청`을 할 수 있을까요?

`REST API` 기반의 테스트를 할 수 있는 [REST-assured](https://rest-assured.io/) 라이브러리를 사용해볼 수 있을 것 같습니다.

**지하철 역을 생성 요청**하는 코드를 통해 살펴볼게요.  

![지하철_역_생성요청](https://github.com/hbkuk/blog/assets/109803585/b8934931-107e-4315-839f-5f59d033278b)
...
![JsonPath 기반 검증](https://github.com/hbkuk/blog/assets/109803585/d54e0622-72d0-480a-91e5-f914d4bfe856)
...
![첫번째_테스트 코드 가독성 높이기](https://github.com/hbkuk/blog/assets/109803585/280af9b4-3722-4804-9340-6942c234b268)
