...
본 포스팅은 [ATDD, 클린 코드 with Spring](https://edu.nextstep.camp/c/R89PYi5H)에서 경험한 **인수 테스트**에 대한 내용에 대해서 소개합니다.
...
인수 테스트에 대해 이야기하기 전, 먼저 **테스트**가 무엇인지 스스로가 고민해볼 필요가 있을 것 같습니다.
...
![인수 테스트 사전적 정의](./images/인수%20테스트%20사전적%20정의.png)  
...
필자의 경험했던 인수 테스트는 아래와 같이 정리할 수 있을 것 같습니다.
...
지금부터는, 필자가 [ATDD, 클린 코드 with Spring](https://edu.nextstep.camp/c/R89PYi5H)에서 경험했던 미션 내용을 기반으로 인수 테스트를 작성하는 방법에 대해서 설명드려고 합니다.

우선, 간단하게 도메인에 대해서 설명드려볼게요.  

### 지하철 도메인 설명

![지하철 도메인 설명](./images/지하철%20도메인.png)  
[참고: ATDD, 클린 코드 with Spring 강의 자료 中 ...](https://edu.nextstep.camp/c/R89PYi5H)

위 그림에서 표현된, 지하철 도메인은 중 지하철 역에 대해서만 이해하고 계시면 될 것 같습니다.
...
일반적으로 지하철 역을 생성하고, 조회하고, 삭제한다. 라는 기능을 구현해본다고 가정해보겠습니다.  

그렇다면, `domain` -> `Repository` -> `Service` -> `Controller` 계층을 순차적으로 설계해볼 것 같아요.  
...
![](./images/테스트코드_없이_검증.png)

인수 테스트를 작성한다면, 위 모든 내용을 테스트 메서드 실행 한번으로 자동화 할 수 있습니다.  
...
인수 테스트에서는 기능을 구현하기 전, 이해당사자와 충분하게 인수 조건을 정의합니다.
...
테스트 진행시, 실제 `Spring Application` 환경에서 주입되는 `Spring Bean` 을 기반으로 진행하기 때문에, `Spring Cotainer`를 구동시켜줘야 하는데요.
...
웹 서버를 구동시킨 후, 어떻게 테스트 코드에서 HTTP 기반 API 요청을 할 수 있을까요?

`REST API` 기반의 테스트를 할 수 있는 [REST-assured](https://rest-assured.io/) 라이브러리를 사용해볼 수 있을 것 같습니다.

아래 해당 라이브러리의 샘플코드를 참고해주세요.

![](./images/Rest_Assured_sample.png)
...
여기서 몇가지 질문을 해볼 수 있는데요.  
생성 요청에 대한 검증은 `Status Code`로 충분할까요?

위에서 인수 테스트 시나리오를 정의할 때 `지하철 역 목록 조회 시 생성한 역을 확인할 수 있다.` 라는 검증 시나리오가 있기 때문에 한번 더 검증이 필요합니다.
...
![](./images/JsonPath%20기반%20검증.png)


지금까지 인수 테스트에 대해서 설명드려보았는데요.  

어떠셨을까요? 괜찮으셨다면, 아래 글도 끝까지 읽어주시면 좋을 것 같습니다.  
...
(`cucumber` 라이브러리 기반으로 리팩토링 하는 방법도 있는데요. 추후 기회가 된다면 포스팅 해보도록 하겠습니다.)

![](./images/첫번째_테스트%20코드%20가독성%20높이기.png)


### 마무리

요근래 깔끔하고 읽기 좋은 프로덕션 혹은 테스트 코드를 작성하려는 노력을 하게되면서, 정말 많이 성장한 것 같다고 느낍니다.

아직 의도적인 훈련이 많이 필요하지만, 꾸준하게 노력한다면 언젠간 더 많이 성장했을 거라고 믿습니다.  

아무쪼록 평소 인수 테스트를 작성해보시지 않았다면, 작성해보시는 것을 추천드립니다.

제대로 한번 배워보고 싶으시다면, [ATDD, 클린 코드 with Spring](https://edu.nextstep.camp/c/R89PYi5H) 강력 추천드립니다.

좋은 경험하게 해주신 **류성현** 강사님께 감사드립니다.
