...
위 실행 계획에서 주요한 내용을 아래와 같이 정리해볼 수 있을 것 같습니다.
...
옵티마이저는 클러스터드 인덱스에서 물리적인 데이터 페이지가 잘 정렬되어 있으니, 이를 활용해서 조회하고 있는 것으로 확인할 수 있습니다.
...
위처럼 비클러스터드 인덱스에서는 클러스터드 인덱스 값을 가지고 있으므로, 추가적으로 클러스터드 인덱스를 통해 다른 컬럼들을 탐색을 하게 됩니다.

옵티마이저 입장에서 생각해본다면, 비클러스터드 인덱스를 사용해야 할 이유가 없지 않을까요?
